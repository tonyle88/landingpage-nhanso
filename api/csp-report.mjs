...
function sanitizeUrl(value) {
  ...
  if (!input) return '';
  if (/^(data|blob|inline|eval|self)$/i.test(input)) return input.toLowerCase();
  try {
...
function normalizeReport(payload) {
  ...
  return {
    ...
    violatedDirective: String(source['violated-directive'] || source.violatedDirective || '').slice(0, 160),
...
export default {
  async fetch(request) {
    ...
    try {
      ...
      reports.forEach((report) => console.info(JSON.stringify(normalizeReport(report))));
    } catch {

...
const client = new pg.Client({
  ...
  application_name: "nhanso-admin-testimonial-verification",
});
...
try {
  ...
  const functions = await client.query(`
    ...
    where pg_namespace.nspname = 'public'
      and proname in ('admin_save_testimonial', 'admin_delete_testimonial')
    order by proname
  `);
  const migration = await client.query(`
    select version
    from supabase_migrations.schema_migrations
    where version in ('202607240005', '202607240006')
    order by version
  `);
  const validation = await client.query(`
    ...
    join pg_namespace on pg_namespace.oid = pg_proc.pronamespace
    where pg_namespace.nspname = 'public'
      and pg_proc.proname = 'admin_save_testimonial'
  `);
  await sample();
  const definition = validation.rows[0]?.definition || "";
  const pass =
    functions.rowCount === 2 &&
    migration.rowCount === 2 &&
    definition.includes("char_length(v_image_url) > 2048") &&
    !definition.includes("{1,2039}");
  if (!pass) throw new Error("Admin testimonial staging verification failed");
  console.log(JSON.stringify({
    status: "PASS",
    target: projectRef,
    migration005: true,
    migration006: true,
    urlValidationFix: true,
    testimonialFunctions: functions.rowCount,

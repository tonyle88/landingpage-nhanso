...
create index if not exists bookings_confirmed_customer_email_idx
  on public.bookings (lower(btrim(email::text)), confirmed_at desc)
...
begin
  ...
  with ranked as (
    select
      ...
      min(coalesce(b.confirmed_at, b.created_at)) over (
        partition by lower(btrim(b.email::text))
      ) as first_booking_at
...
revoke all on function public.admin_list_booking_customers(text, integer, integer)
  from public;
grant execute on function public.admin_list_booking_customers(text, integer, integer)
  to authenticated;

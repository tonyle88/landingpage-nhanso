...
import { DEFAULT_SURVEY_INTRO, DEFAULT_SURVEY_QUESTIONS, DEFAULT_SURVEY_TITLE, parseSurveyQuestions } from "@/lib/survey";
...
export async function saveSurveyAction(form: FormData) {
  ...
  if (!UUID.test(id) || title.length < 5 || title.length > 160 || intro.length < 10 || intro.length > 1200 || !questions) {
    redirect(`/admin/surveys?${UUID.test(id) ? `id=${id}&` : ""}status=invalid`);
  }

  const supabase = await createAuthServerClient();
  const { data, error } = await supabase.from("service_surveys")
    .update({ title, intro, questions, active })
    .eq("id", id)
    .select("id")
    .maybeSingle();
  if (error || !data) {
